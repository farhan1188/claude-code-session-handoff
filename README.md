...
Two parts: an [Agent Skill](https://agentskills.io) — one `SKILL.md` Claude loads when the work
matches — and one optional `SessionStart` hook, a 150-line bash script. The skill is instructions;
the hook is the only part that runs deterministically.
...
conversation, including a rule the project's own config still contradicted. A session that judges its
own handoff will pass itself. A grep will not.
...
When compaction is right, it hands you a targeted `/compact` command — what to preserve, what to
discard — rather than the bare one that lets the summarizer choose for you.
...
The skill reconciles the two — `git status`, real diffs, files and symbols actually touched, what was
verified versus only implemented — and where they disagree, the repository wins and the disagreement
is recorded.
...
| Completed | Re-doing work that exists — and, by separating implementation from verification, trusting work nobody checked |
...
from `CLAUDE.md` or a rule file. If it does, the prompt leaves it out — a rule in both places is a
